# App: iterated scientific hello world example.
# Problem: relation between callback and yield/iterator.

from math import sin, pi

def simulator(n, callback):
    print 'in simulator'
    for i in range(n):
        r = i*pi/(n-1)
        s = sin(r)
        print 'in simulator, s', s
        callback(r, s)

def mycompute():
    print 'in mycompute'
    def mycallback(r, s):
        result = 'Hello, World! sin(%g)=%g' % (r, s)
        print 'in mycallback', result
        #yield text  # doesn't work
